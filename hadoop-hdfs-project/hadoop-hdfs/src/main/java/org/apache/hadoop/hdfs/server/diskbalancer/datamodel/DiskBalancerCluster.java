...
public class DiskBalancerCluster {
  ...
  public List<NodePlan> computePlan(double thresholdPercent){
    return this.computePlan(PlannerFactory.GREEDY_PLANNER,thresholdPercent);
  }

  public List<NodePlan> computePlan(String type, double thresholdPercent) {
    ...
    for (int x = 0; x < nodesToProcess.size(); x++) {
      ...
      final Planner planner = PlannerFactory
              .getPlanner(type, node, thresholdPercent);
